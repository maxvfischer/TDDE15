...
Z_particles_t_1 <- runif(n = N, min = 0, max = 100) # Initial particles (hypothesis of robot states)
Z_particles_t <- rep(0, N) # Array that will be filled with new hypothesis
weights <- rep(0, N)
...
for (t in 1:T) {
  ...
  for (n in 1:N) {
    ...
    weights[n] <- emission_density(data$X_true[t], Z_particles_t[n], 1)
  }
...
Z_5_particles_t_1 <- runif(n = N, min = 0, max = 100) # Initial particles (hypothesis of robot states)
Z_5_particles_t <- numeric(0) # Array that will be filled with new hypothesis
Z_50_particles_t_1 <- runif(n = N, min = 0, max = 100)
Z_50_particles_t <- rep(0, N)
weights_5 <- rep(0, N)
weights_50 <- rep(0, N)
...
for (t in 1:T) {
  ...
  for (n in 1:N) {
    ...
    # Generate new particle (state hypothesis) from old one. p(z_t | z_t_1)
    Z_5_particles_t[n] <- transition_model(Z_5_particles_t_1[n])
    Z_50_particles_t[n] <- transition_model(Z_50_particles_t_1[n])
    
    # Generate weight for the new hypothesis.
    # How likely is the observation in this time step, 
    # given the new hypothesis state (robot position)
    weights_5[n] <- emission_density(data_5$X_true[n], Z_5_particles_t[n], sigmas[1])
    weights_50[n] <- emission_density(data_50$X_true[n], Z_50_particles_t[n], sigmas[2])
  }
  
  # Sample 100 particles (hypothesis), with respect to their
  # weight (relative "probability" among the particles)
  Z_5_particles_t_1 <- sample(x = Z_5_particles_t,
                            size = 100,
                            prob = weights_5,
                            replace = TRUE)
  Z_50_particles_t_1 <- sample(x = Z_50_particles_t,
                            size = 100,
                            prob = weights_50,
                            replace = TRUE)
}
...
par(mfrow=c(2,2))
hist(Z_5_particles_t_1)
hist(data_5$Z_true)
hist(Z_50_particles_t_1)
hist(data_50$Z_true)
